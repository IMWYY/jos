...
alloc_block(void)
{
	...
	for (size_t block_i = 0; block_i < super->s_nblocks; block_i++)
	{
		if (block_is_free(block_i)) {
			bitmap[block_i/32] &= ~(1<<(block_i%32));

			flush_block(diskaddr(2 + block_i / BLKBITSIZE));
			return block_i;
		}
	}
	return -E_NO_DISK;
...
{
	if (filebno >= NDIRECT && !alloc) return -E_NOT_FOUND;
