...
trap_init_percpu(void)
{
	...
	int c_i = thiscpu->cpu_id;
	thiscpu->cpu_ts.ts_esp0 = KSTACKTOP - c_i * (KSTKSIZE + KSTKGAP);
	thiscpu->cpu_ts.ts_ss0 = GD_KD;
	thiscpu->cpu_ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + c_i] = SEG16(STS_T32A, (uint32_t) (&(thiscpu->cpu_ts)),
					sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + c_i].sd_s = 0;
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall) {
		...
		if (tf->tf_esp < UXSTACKTOP && tf->tf_esp >= UXSTACKTOP - PGSIZE) {
			utf_start = tf->tf_esp - sizeof(struct UTrapframe) - 4;
		} else {
			utf_start = UXSTACKTOP - sizeof(struct UTrapframe);
		}
		user_mem_assert(curenv, (void *)utf_start, 
			sizeof(struct UTrapframe), PTE_W | PTE_U);

		utf = (struct UTrapframe *)utf_start;
