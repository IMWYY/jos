...
sys_cputs(const char *s, size_t len)
{
	...
	// cprintf("tf_cs: %d, s: %08x, len: %d \n", (curenv->env_tf.tf_cs & 3), s, len);
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	int ret = envid2env(srcenvid, &src_e, 1);
	if (ret) return ret;
	if (!src_e) return -E_BAD_ENV;
	ret = envid2env(dstenvid, &dest_e, 1);
	if (ret) return ret;
	if (!dest_e) return -E_BAD_ENV;
	...
	pte_t *pt;
	struct PageInfo *pp = page_lookup(src_e->env_pgdir, srcva, &pt);
	//	-E_INVAL is srcva is not mapped in srcenvid's address space.
	if (!pt || (*pt & PTE_P) != PTE_P) return -E_INVAL;
	...
	//	-E_NO_MEM if there's no memory to allocate any necessary page tables.
	ret = page_insert(dest_e->env_pgdir, pp, dstva, perm);
	return ret;
}
