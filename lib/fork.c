...
pgfault(struct UTrapframe *utf)
{
	...
	// cprintf("pgfault err: %01x, addr: %08x \n", err, addr);
	if (!(err & FEC_WR)) {
		panic("pgfault: not a FEC_WR.");
	}
	...
	if (sys_page_alloc(0, (void *)PFTEMP, PTE_U | PTE_P | PTE_W)) {
		panic("pgfault: sys_page_alloc fail.");
	}
	memcpy(PFTEMP, addr, PGSIZE);
	if (sys_page_map(0, (void *)PFTEMP, 0, addr, PTE_U | PTE_W | PTE_P)) {
		panic("pgfault: sys_page_map fail.");
	}

	if (sys_page_unmap(0, (void *)PFTEMP)) {
		panic("pgfault: sys_page_umap fail.");
	}
}
...
fork(void)
{
	...
	if (envid < 0) panic("fork: call sys_exofork fail.");
	...
	if (sys_page_alloc(envid, (void *)(UXSTACKTOP - PGSIZE), PTE_U |PTE_P |PTE_W)) {
		panic("fork: call sys_page_alloc fail");
	}
	...
	if (sys_env_set_status(envid, ENV_RUNNABLE)) {
		panic("fork: call sys_env_set_status fail.");
	}
